...
RSpec.describe Agent::OrdersController, type: :controller do
  ...
  # Agent::OrdersController. Be sure to keep this updated too.
  ...
  describe "POST #create" do
    context "with valid params" do
      ...
      it "redirects to the created agent_order" do
        post :create, params: {order: valid_attributes}
        expect(response).to redirect_to(agent_order_url(Order.last))
      end
    end
  ...
  describe "PUT #update" do
    context "with valid params" do
      ...
      it "updates the requested agent_order" do
        order = create(:order)
        put :update, params: {id: order.to_param, order: new_attributes}
        order.reload
        skip("Add assertions for updated state")
      end
      it "redirects to the agent_order" do
        order = create(:order)
        put :update, params: {id: order.to_param, order: valid_attributes}
        expect(response).to redirect_to(agent_order_url(order))
      end
    end
    context "with invalid params" do
  ...
  describe "DELETE #destroy" do
    it "destroys the requested agent_order" do
      order = create(:order)
      expect {
        delete :destroy, params: {id: order.to_param}
      }.to change(Order, :count).by(-1)
    end
    it "redirects to the agent_orders list" do
      order = create(:order)
      delete :destroy, params: {id: order.to_param}
